...
public class ImmunotherapyService {
    ...
    public Immunotherapy updateImmunotherapy(AddImmunotherapyRequest addImmunotherapyRequest, Long immunotherapyId){
        Optional<Immunotherapy> optionalImmunotherapy = immunotherapyRepository.findById(immunotherapyId);
        if (optionalImmunotherapy.isPresent()) {
            Immunotherapy immunotherapy = optionalImmunotherapy.get();
            immunotherapy = addImmunotherapyRequest.mapToImmunotherapy(doctorService, userService, hospitalService, immunotherapy);
            immunotherapyRepository.save(immunotherapy);
            return immunotherapy;
        }
        return null;
