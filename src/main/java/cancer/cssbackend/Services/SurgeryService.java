...
public class SurgeryService {
    ...
    public Surgery updateSurgery(AddSurgeryRequest addSurgeryRequest, Long surgeryId) {
        Optional<Surgery> optionalSurgery = surgeryRepository.findById(surgeryId);
        if (optionalSurgery.isPresent()) {
            Surgery surgery = optionalSurgery.get();
            surgery = addSurgeryRequest.mapToSurgery(doctorService, hospitalService, userService, surgery);
            surgeryRepository.save(surgery);
            return surgery;
        }
        return null;
